# from Nuaza.checkout import google_checkout
from Nuaza.cart import cart
from Nuaza.checkout.models import Order, OrderItem
from Nuaza.checkout.forms import CheckoutForm
import random
import string
# from Nuaza.checkout import authnet
# from Nuaza import settings

from django.core import urlresolvers
# import urllib

# Generate a random and unique transaction ID

# def generate_transation_id(request):
...
def process(request):
    ...
    if response[0] == APPROVED:
        ...
        transaction_id = 'NSThYzAD'
	#used_transaction_ids = Order.objects.filter(transaction_id==transaction_id)
	
	while (Order.objects.filter(transaction_id=transaction_id)):
		transaction_id = ''.join(random.choice(string.ascii_letters + string.digits) for letter in xrange(8))
        
        order = create_order(request, transaction_id)
        results = {'order_number': order.id, 'message': u''}
    if response[0] == DECLINED:
        results = {'order_number': 0, 'message': u'There is a problem with your order.'}
    if response[0] == ERROR or response[0] == HELD_FOR_REVIEW:
        results = {'order_number': 0, 'message': u'There was an Error processing your order.'}
    return results
